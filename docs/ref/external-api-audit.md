# External API Audit — SignalCraft API (staging v0.0.1)

Last Updated: 2026-06-05
Source: `https://signalcraft-api-55721952249.asia-northeast3.run.app/openapi.json`
Scope: Cloud Run (asia-northeast3) FastAPI 서비스. ON/OFF (L1) 검증 + 대시보드 연동용 외부 API.

> 본 문서는 **외부 Cloud Run API** 사양 감사 결과이며, Railway에 배포된 `backend/`(내부 FastAPI)와는 별개의 서비스입니다.
...
| GET | `/me` | 사용자 관련 | `MeResponse` | 컨텍스트 부트스트랩 |
| GET | `/machines` | machine | `MachinesResponse` | 현재 상태 스냅샷 |
| GET | `/machines/{machine_id}` | machine | `MachineDetailResponse` | 시계열 + `period` 쿼리 필수 |
